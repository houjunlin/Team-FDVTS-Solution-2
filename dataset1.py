...
class Lung3D_eccv_patient_supcon(Data.Dataset):
            ...
            if train:
                ...
                for iter, rows in csv_file.iterrows():
                    ...
                    if box_lung:
                        ...
                        self.datalist.append((img, int(grade)-1, name, '/home/feng/hjl/eccv-submit/segment/eccv_seg/severity/lesiononly_mask/train/'+name+'.npy'))
            ...
            if val:
                ...
                for iter, rows in csv_file.iterrows():
                    ...
                    if box_lung:
                        ...
                        self.datalist.append((img, int(grade)-1, name, '/home/feng/hjl/eccv-submit/segment/eccv_seg/severity/lesiononly_mask/val/'+name+'.npy'))
                    else:
                        self.datalist.append((img, int(grade)-1, name))
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            ...
            # # cv2.imwrite('debug.png',new*255)
            # cv2.imwrite('lesioncat/img_nonorm/'+ID+'.png',new*255)
            # cv2.imwrite('lesioncat/mask/'+ID+'.png',newmask*255)
...
class TwoCropTransform2:
    ...
    def __call__(self, x, mask):
        x1, mask1 = self.transform(x, mask, self.ifhalfcrop)
        x2, mask2 = self.transform(x, mask, self.ifhalfcrop)
...
def augment2(sample, mask, ifhalfcrop=False,ifrandom_resized_crop=True, ifflip=False, ifrotate=False, ifcontrast=True,ifswap = False,filling_value=0):
    if ifhalfcrop:
        start = np.random.randint(0,sample.shape[0]-64)
        sample = sample[start:start+64]
    if ifrandom_resized_crop:
